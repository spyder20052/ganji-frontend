...
export function Logo({ href = '/', light = false }: { href?: string; light?: boolean }) {
  return (
    <Link prefetch={false} href={href} className="inline-flex items-center gap-2 font-bold text-xl tracking-tight" aria-label="Ganji, accueil">
      ...
      <span className={light ? 'text-white' : 'text-[var(--color-brand-900)] dark:text-[var(--fg)]'}>Ganji</span>
    </Link>
  );
}
