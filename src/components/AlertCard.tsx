  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="pill border border-[var(--border)]"><Pictogram name={kind.icon} size={14} /> {kind.label}</span>
        {alert.national ? (
          <span className="pill border border-[var(--border)]"><Globe2 size={14} aria-hidden /> National</span>
        ) : (
          <span className="pill border border-[var(--border)]"><MapPin size={14} aria-hidden /> {alert.communes.join(', ')}</span>
        )}
        {alert.auto && <span className="pill bg-[var(--color-ocre-500)] text-[var(--color-ink)]"><Bot size={14} aria-hidden /> Détection automatique</span>}
        {fresh && <span className="pill bg-[var(--color-brand-900)] text-white">Nouveau</span>}
      </div>
      <h3 id={`al-${alert.id}`} className="text-xl font-bold">{alert.title}</h3>
      <p>{alert.message}</p>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-[var(--fg-muted)]">{alert.source} · {relative(alert.createdAt)}</p>
        {listen && <ListenButton text={`${sev.label}. ${alert.title}. ${alert.message}`} audioKey={alert.audioKey ?? undefined} />}
      </div>
    </article>
  );
}
