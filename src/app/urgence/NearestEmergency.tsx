  ...
  return (
    <section aria-labelledby="h-near" className="card space-y-4 p-5">
      <div className="flex items-center gap-3">
        ...
        <h2 id="h-near" className="text-xl font-bold">Hôpital ouvert le plus proche</h2>
      </div>
      <LocateControl idPrefix="urg" status={geo.status} source={geo.source} onLocate={geo.request} onCommune={(p) => geo.setManual(p)} />
      {loading && (
        <p className="flex items-center gap-2 text-[var(--fg-muted)]" role="status"><Loader2 className="animate-spin" aria-hidden /> Recherche des urgences ouvertes…</p>
      )}
      {error && <p role="alert" className="font-bold">{error}</p>}
      {places && !loading && (
        <div aria-live="polite">
          <PlaceList places={places} from={geo.source === 'gps' ? geo.pos : null} empty="Aucune urgence ouverte trouvée. Appelez le 118." />
        </div>
      )}
    </section>
  );
