  ...
  async function send() {
    ...
    try {
      const r = await sendOrQueue<ReportResult>({
        ...
        method: 'POST',
        body,
        label: `Signalement : ${chosen.label.toLowerCase()}, ${cases} cas, ${village.trim() || commune}`,
      });
      setOutcome(r);
      setStep('done');
      try {
  ...
  return (
    <>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="label">Relais communautaire · surveillance</p>
          <h1 className="mt-1 text-3xl font-bold">Signaler en 3 gestes</h1>
        </div>
        {pending.length > 0 && (
          <button type="button" onClick={() => void flush()} className="pill !py-2 bg-[var(--color-ocre-100)] text-[var(--color-ocre-700)]" disabled={!online}>
            <CloudOff size={16} aria-hidden /> {pending.length} en attente d’envoi {online && <RefreshCw size={14} aria-hidden />}
          </button>
        )}
      </div>
      ...
      {!online && (
        <p className="flex items-start gap-3 rounded-2xl bg-[var(--color-ocre-100)] p-4 font-bold text-[var(--color-ocre-700)]" role="status">
          ...
          Hors ligne : vos signalements sont gardés sur le téléphone et partiront au retour du réseau.
        </p>
      )}
      {flushed > 0 && (
        <p className="flex items-center gap-2 rounded-2xl bg-[var(--color-brand-100)] p-4 font-bold text-[var(--color-brand-900)]" role="status">
          <CheckCircle2 size={22} aria-hidden /> Réseau revenu : {flushed} signalement{flushed > 1 ? 's' : ''} envoyé{flushed > 1 ? 's' : ''}.
        </p>
      )}
      ...
      <section aria-labelledby="h-step" className="card space-y-5 p-5 sm:p-6">
        <ol className="grid grid-cols-3 gap-2" aria-label="Étapes">
          {[1, 2, 3].map((n) => (
            <li key={n} className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-bold ${n <= stepNum ? 'bg-[var(--color-brand-900)] text-white' : 'bg-[var(--bg)] text-[var(--fg-muted)]'}`} aria-current={n === step ? 'step' : undefined}>
              <span className="num">{n}</span> {n === 1 ? 'Quoi' : n === 2 ? 'Combien' : 'Où'}
            </li>
          ))}
        </ol>
        ...
        {step === 1 && (
          <>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 id="h-step" className="text-2xl font-bold">Qu’avez-vous vu ?</h2>
              <ListenButton text={`${STEP_TEXT[1]} ${SYNDROMES.map((s) => s.label).join(', ')}.`} audioKey="relay.step1" />
            </div>
        ...
        {step === 2 && chosen && (
          <>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 id="h-step" className="text-2xl font-bold">Combien de personnes malades ?</h2>
              <ListenButton text={STEP_TEXT[2]} audioKey="relay.step2" />
            </div>
            <Chosen icon={chosen.icon} label={chosen.label} />
            <div className="flex items-center justify-center gap-6 py-2">
              <button type="button" onClick={() => setCases((n) => Math.max(1, n - 1))} disabled={cases <= 1} className="grid h-20 w-20 place-items-center rounded-full border-2 border-[var(--border)] bg-[var(--card)] disabled:opacity-40" aria-label="Une personne de moins">
                <Minus size={36} aria-hidden />
              </button>
              <output className="num w-28 text-center text-7xl font-bold" aria-live="polite" aria-label={`${cases} personne${cases > 1 ? 's' : ''}`}>{cases}</output>
              <button type="button" onClick={() => setCases((n) => Math.min(200, n + 1))} className="grid h-20 w-20 place-items-center rounded-full bg-[var(--color-brand-900)] text-white" aria-label="Une personne de plus">
                <Plus size={36} aria-hidden />
              </button>
            </div>
            <div className="flex flex-wrap justify-center gap-2" aria-label="Raccourcis">
              {[1, 2, 5, 10].map((n) => (
                <button key={n} type="button" onClick={() => setCases(n)} className={`btn !min-h-11 !px-5 ${cases === n ? 'btn-soft' : 'btn-ghost'}`}>{n}</button>
              ))}
            </div>
            <div className="flex gap-3">
              <button type="button" className="btn btn-ghost" onClick={() => setStep(1)}><ArrowLeft size={20} aria-hidden /> Retour</button>
              <button type="button" className="btn btn-primary flex-1" onClick={() => setStep(3)}>Suivant</button>
            </div>
          </>
        )}
        ...
        {step === 3 && chosen && (
          <>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 id="h-step" className="text-2xl font-bold">Où ?</h2>
              <ListenButton text={STEP_TEXT[3]} audioKey="relay.step3" />
            </div>
            <Chosen icon={chosen.icon} label={`${chosen.label} · ${cases} personne${cases > 1 ? 's' : ''}`} />
            <label className="block">
              <span className="label mb-1.5 block">Village ou quartier</span>
              <input className="input !min-h-14 text-lg" value={village} maxLength={60} onChange={(e) => setVillage(e.target.value)} placeholder="Ex. Kolokondé" autoComplete="off" />
            </label>
            <CommuneSelect id="relay-commune" value={commune} onChange={(n) => n && setCommune(n)} label="Commune" allowEmpty={false} />
            {IMMEDIATE.has(chosen.code) && (
              <p className="flex items-start gap-2 rounded-2xl bg-[var(--color-danger-50)] p-3 font-bold text-[var(--color-danger-800)]">
                <Siren size={20} className="mt-0.5 shrink-0" aria-hidden /> Signe à déclarer tout de suite : l’équipe de la zone sanitaire sera prévenue dès l’envoi.
              </p>
            )}
            {error && <p role="alert" className="rounded-2xl bg-[var(--color-ocre-100)] p-3 font-bold text-[var(--color-ocre-700)]">{error}</p>}
            <div className="flex gap-3">
              <button type="button" className="btn btn-ghost" onClick={() => setStep(2)}><ArrowLeft size={20} aria-hidden /> Retour</button>
              <button type="button" className="btn btn-primary !min-h-14 flex-1 text-lg" onClick={send} disabled={sending || !commune}>
                {sending ? <Loader2 size={22} className="animate-spin" aria-hidden /> : <Send size={22} aria-hidden />} Envoyer le signalement
              </button>
            </div>
          </>
        )}
        ...
        {step === 'done' && outcome && (
          <div className="space-y-4" aria-live="polite">
            <h2 id="h-step" className="sr-only">Résultat</h2>
            {outcome.queued ? (
              <div className="flex items-start gap-3 rounded-2xl bg-[var(--color-ocre-100)] p-5 text-[var(--color-ocre-700)]">
                ...
                <div>
                  <p className="text-xl font-bold">Signalement gardé sur le téléphone</p>
                  <p className="font-bold">Il sera envoyé au retour du réseau, avec la date et l’heure d’aujourd’hui.</p>
                </div>
              </div>
            ) : (
              <div className="flex items-start gap-3 rounded-2xl bg-[var(--color-brand-100)] p-5 text-[var(--color-brand-900)]">
                ...
                <div>
                  <p className="text-xl font-bold">Signalement envoyé. Merci !</p>
                  <p>L’équipe de la zone sanitaire le voit tout de suite.</p>
                </div>
              </div>
            )}
            {outcome.result?.clusterAlert && (
              <div className="space-y-2 rounded-2xl border-2 border-[var(--color-ocre-500)] p-5" role="alert">
                <p className="flex items-center gap-2 text-xl font-bold"><Siren size={24} aria-hidden /> Alerte envoyée au médecin chef de zone</p>
                <p>Plusieurs signalements du même type dans votre commune : une investigation est demandée.</p>
            ...
            <ListenButton
              text={
                outcome.queued
                  ? 'Signalement gardé sur le téléphone. Il partira au retour du réseau.'
                  : `Signalement envoyé. Merci.${outcome.result?.clusterAlert ? ' Alerte envoyée au médecin chef de zone.' : ''}`
              }
              audioKey={outcome.queued ? 'relay.queued' : 'relay.sent'}
            />
            <button type="button" className="btn btn-primary w-full" onClick={reset}>Nouveau signalement</button>
          </div>
        )}
      </section>
      ...
      <section id="activite" aria-labelledby="h-activity" className="scroll-mt-24 space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 id="h-activity" className="text-xl font-bold">À {commune} <span className="text-base font-normal text-[var(--fg-muted)]">· 14 derniers jours</span></h2>
          <button type="button" className="chip-round" onClick={loadActivity} aria-label="Actualiser" disabled={!online}><RefreshCw size={18} aria-hidden /></button>
        </div>
            ...
            {pending.map((q) => (
              <li key={q.id} className="card flex items-center gap-3 border-dashed p-4">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-sm text-[var(--fg-muted)]">Saisi {fmtDateTime(q.createdAt)} · en attente de réseau</span>
                </span>
              </li>
            ))}
          </ul>
        ...
        {activityError && <p className="text-[var(--fg-muted)]">Historique indisponible pour le moment.</p>}
        {!reports && !activityError && online && <p className="text-[var(--fg-muted)]" role="status">Chargement…</p>}
        {reports && <ReportList reports={reports.filter((r) => r.commune === commune)} />}

        {alerts && alerts.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-lg font-bold">Alertes pour {commune}</h3>
...
function ReportList({ reports }: { reports: CommunityReport[] }) {
  if (!reports.length) return <p className="text-[var(--fg-muted)]">Aucun signalement récent dans cette commune.</p>;
    ...
    <ul className="space-y-2">
      {reports.slice(0, 20).map((r) => (
        <li key={r.id} className="card flex items-center gap-3 p-4">
          ...
          <span className="min-w-0 flex-1">
            <span className="block font-bold">{r.label} · <span className="num">{r.cases}</span> cas</span>
            <span className="block text-sm text-[var(--fg-muted)]">{r.village ? `${r.village} · ` : ''}{fmtDateTime(r.at)}{r.relay ? ` · ${r.relay}` : ''}</span>
          </span>
        </li>
      ))}
    </ul>
