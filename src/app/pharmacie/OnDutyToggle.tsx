...
export function OnDutyToggle({ initial }: { initial: boolean }) {
  ...
  async function toggle() {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : t('Changement non enregistré. Réessayez.'));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section aria-labelledby={labelId} className="card space-y-3 p-5">
      ...
      <p role="status" className={on ? 'font-bold text-[var(--color-brand-900)] dark:text-[var(--color-brand-200)]' : 'text-[var(--fg-muted)]'}>
        {on ? t('De garde : vous apparaissez en tête sur la carte et dans « Trouver un médicament ».') : t('Hors garde : activez la garde quand votre tour commence.')}
      </p>
