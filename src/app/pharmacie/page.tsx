...
export default async function PharmaciePage() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-3xl font-bold">{stock?.pharmacy.name ?? 'Ma pharmacie'}</h1>
              ...
              <ul className="mt-2 space-y-1 text-[0.95rem]">
                {ruptures.slice(0, 8).map((r) => (
                  <li key={r.medication.id}>
                    <span className="font-bold">{r.medication.dci}</span> {r.medication.strength} <span className="text-[var(--fg-muted)]">· {r.medication.form}</span>
                  </li>
                ))}
              </ul>
              {ruptures.length > 8 && <p className="mt-1 text-sm text-[var(--fg-muted)]">et {ruptures.length - 8} autres, dans le tableau ci-dessous.</p>}
            </section>
          )}
        </div>
      </div>
      {stock ? <StockTable initial={stock.items} /> : <p className="card p-5 text-[var(--fg-muted)]">Stock indisponible pour le moment.</p>}
    </div>
  );
}
