...
export function PersonaTabs({ personas, available }: { personas: PersonaView[]; available: Record<string, boolean> }) {
  ...
  return (
    <div className="space-y-6">
      <div role="tablist" aria-label={t('Profils')} className="-mx-4 flex snap-x scroll-px-4 gap-2 overflow-x-auto px-4 pb-2 md:mx-0 md:grid md:grid-cols-4 md:overflow-visible md:px-0">
        {personas.map((x, i) => {
            ...
            >
              <span className="block h-10 w-10 overflow-hidden rounded-full bg-brand-100">
                <IllustrationView
                  ...
                  fallbackIcon={<span className="text-xs font-semibold">{x.name.replace(/^(Dr|La|Le|The) /, '')[0]}</span>}
                />
              </span>
      ...
      <div key={p.key} role="tabpanel" id="panel-persona" aria-labelledby={`tab-${p.key}`} className="panel-in grid gap-6 rounded-card bg-card p-5 md:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)] md:items-center md:gap-10 md:p-8">
        <div className="overflow-hidden rounded-[8%]">
          <IllustrationView
            name={`persona-${p.key}`}
            frame="aspect-square"
            alt={t('Portrait : {nom}, {role}', { nom: p.name, role: p.role })}
        ...
        <div className="space-y-5">
          ...
          <button type="button" onClick={() => void tryAs()} disabled={busy} className="btn btn-primary">
            {busy ? t('Connexion…') : t('Essayer comme {nom}', { nom: p.name })} <ArrowRight size={18} aria-hidden />
          </button>
          {error && (
