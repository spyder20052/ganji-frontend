...
export function Simulator({ initialTel }: { initialTel: string }) {
  ...
  async function runTick() {
    ...
    try {
      ...
      setTick({
        loading: false,
        text: `${r.remindersSent} rappel${r.remindersSent > 1 ? 's' : ''} envoyé${r.remindersSent > 1 ? 's' : ''} (SMS, voix, application) · ${r.donorAlertsExpired} appel${r.donorAlertsExpired > 1 ? 's' : ''} au don expiré${r.donorAlertsExpired > 1 ? 's' : ''} · ${r.clusterAlerts} alerte${r.clusterAlerts > 1 ? 's' : ''} de regroupement`,
      });
      loadInbox();
      loadFeed();
    } catch (e) {
  ...
  const label = useMemo(() => {
    ...
    return donor ? `Donneur · ${donor.label}` : 'Numéro libre';
  }, [phones, phone]);
    ...
    <>
      <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
        ...
        <div className="space-y-2">
          <button type="button" className="btn btn-primary w-full" onClick={runTick} disabled={tick.loading || demoOff}>
            ...
            Envoyer les rappels du jour maintenant
          </button>
      ...
      {demoOff && (
        <p className="rounded-2xl bg-[var(--color-ocre-100)] p-4 font-bold text-[var(--color-ocre-700)]" role="alert">
          Le simulateur n’est disponible qu’en mode démonstration (DEMO_MODE=true côté API).
        </p>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)_320px]">
        ...
        <section aria-labelledby="h-phones" className="space-y-4">
          <h2 id="h-phones" className="text-lg font-bold">Quel téléphone ?</h2>
          <form onSubmit={submitFree} className="flex gap-2">
            <label htmlFor="sim-tel" className="sr-only">Numéro de téléphone</label>
            ...
            <button type="submit" className="btn btn-soft shrink-0 !px-4">Voir</button>
          </form>
          {!phones && <p className="text-base text-[var(--fg-muted)]" role="status">Chargement…</p>}
          {phones && phones.donors.length > 0 && (
            <PhoneGroup title="Donneurs alertés">
              {phones.donors.map((d) => (
          ...
          {phones && phones.personas.length > 0 && (
            <PhoneGroup title="Personnes de la démo">
              {phones.personas.map((p) => (
                <PhoneButton key={p.phone} active={p.phone === phone} onClick={() => select(p.phone)} icon="user" title={p.label} sub={`${ROLE[p.role] ?? p.role} · ${fmtPhone(p.phone)}`} />
              ))}
            </PhoneGroup>
          )}
        </section>
        ...
        <section aria-labelledby="h-device" className={`space-y-3 ${phone ? 'max-lg:order-first' : ''}`}>
          <h2 id="h-device" className="sr-only">Écran du téléphone {phone ? fmtPhone(phone) : ''}</h2>
          <div className="mx-auto w-full max-w-[360px] rounded-[2.75rem] border-[10px] border-[#1b2622] bg-[#1b2622] shadow-2xl">
            <div className="flex h-[640px] flex-col overflow-hidden rounded-[2.1rem] bg-[#f4f7f5] text-[#13241e]">
              ...
              <div role="tablist" aria-label="Canal" className="grid grid-cols-2 border-b border-[#d5e0da] bg-white text-sm font-bold">
                {(['sms', 'ussd'] as const).map((t) => (
                  <button
                    key={t}
                    role="tab"
                    type="button"
                    ...
                    id={`tab-${t}`}
                    onClick={() => setTab(t)}
                    className={`min-h-11 border-b-2 ${tab === t ? 'border-[#0b4f3c] text-[#0b4f3c]' : 'border-transparent text-[#52635b]'}`}
                  >
                    {t === 'sms' ? 'Messages et appels' : `USSD ${USSD_CODE}`}
                  </button>
                ))}
              </div>
              {tab === 'sms' ? (
          ...
          <p className="text-center text-sm text-[var(--fg-muted)]">Actualisation automatique toutes les 3 secondes.</p>
        </section>

        {/* ── Tous les messages ── */}
        <section aria-labelledby="h-feed" className="space-y-3">
          <h2 id="h-feed" className="text-lg font-bold">Tous les messages <span className="num text-base font-normal text-[var(--fg-muted)]">({feed.length})</span></h2>
          <ul className="card max-h-[680px] divide-y divide-[var(--border)] overflow-y-auto">
            {feed.length === 0 && <li className="p-4 text-base text-[var(--fg-muted)]">Aucun message envoyé pour le moment.</li>}
            {feed.map((m) => (
              <li key={m.id} className="space-y-1 p-3 text-sm">
                <div className="flex items-center gap-2">
                  <ChannelIcon channel={m.channel} />
                  <button type="button" onClick={() => select(m.to)} className="num font-bold underline-offset-2 hover:underline" title="Afficher ce téléphone">
                    {fmtPhone(m.to)}
                  </button>
                  <span className="ml-auto num text-[var(--fg-muted)]">{fmtTime(m.createdAt)}</span>
                </div>
...
function ChannelIcon({ channel }: { channel: string }) {
  if (channel === 'VOICE') return <Phone size={16} aria-label="Appel vocal" className="shrink-0 text-[var(--color-brand-500)]" />;
  if (channel === 'PUSH') return <Bell size={16} aria-label="Notification" className="shrink-0 text-[var(--color-ocre-700)]" />;
...
function StatusBar() {
  ...
  useEffect(() => {
    const up = () => setNow(fmtTime(new Date()));
    up();
  ...
  return (
    <div id="panel-sms" role="tabpanel" aria-labelledby="tab-sms" className="flex min-h-0 flex-1 flex-col">
      <div ref={scroller} className="flex-1 space-y-3 overflow-y-auto px-3 py-4" aria-live="polite" aria-relevant="additions">
        {!phone && <p className="p-4 text-center text-sm text-[#52635b]">Choisissez un téléphone à gauche.</p>}
        {phone && thread.length === 0 && <p className="p-4 text-center text-sm text-[#52635b]">Aucun message reçu sur ce numéro pour le moment.</p>}
        {thread.map((t) => {
          if (t.kind === 'out') {
            return (
              <div key={t.sms.id} className="ml-auto max-w-[80%] space-y-1">
                <p className="rounded-2xl rounded-br-md bg-[#0b4f3c] px-3 py-2 text-[15px] text-white">{t.sms.body}</p>
                <p className="text-right text-[11px] text-[#52635b]">
                  {fmtTime(t.sms.at)} · {t.sms.error ? `Échec : ${t.sms.error}` : t.sms.handled ? (HANDLED[t.sms.handled] ?? t.sms.handled) : 'Envoi…'}
                </p>
              </div>
            );
          }
            ...
            return (
              <div key={m.id} className="space-y-2 rounded-2xl border border-[#c3e2d3] bg-[#e3f1eb] p-3">
                <p className="flex items-center gap-2 text-sm font-bold text-[#0b4f3c]">
                  <PhoneCall size={16} aria-hidden /> Appel vocal ({LANG[m.lang] ?? m.lang})
                  <span className="ml-auto num text-[11px] font-normal text-[#52635b]">{fmtTime(m.createdAt)}</span>
                </p>
                <p className="text-[14px] text-[#13241e]">{voiceText(m.body)}</p>
                <ListenButton text={voiceText(m.body)} audioKey={m.audioKey ?? undefined} lang={m.lang} label="Écouter l’appel" />
              </div>
            );
          }
          if (m.channel === 'PUSH') {
          ...
          return (
            <div key={m.id} className="max-w-[85%] space-y-1">
              ...
              <p className="text-[11px] text-[#52635b]">SMS · {fmtTime(m.createdAt)}</p>
            </div>
          );
        })}
      </div>
      <div className="space-y-2 border-t border-[#d5e0da] bg-white p-3">
        <div className="flex gap-2" aria-label="Réponses rapides">
          {['1', '2', 'RDV'].map((q) => (
        ...
        >
          <label htmlFor="sms-draft" className="sr-only">Répondre par SMS</label>
          <input
            id="sms-draft"
          ...
          />
          <button type="submit" disabled={!phone || !draft.trim()} className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#0b4f3c] text-white disabled:opacity-50" aria-label="Envoyer le SMS">
...
function UssdPanel({ phone }: { phone: string }) {
  ...
  async function post(text: string) {
    ...
    try {
      ...
      setScreen(`Erreur de connexion : ${(e as Error).message}`);
      setStage('ended');
    } finally {
      setLoading(false);
      setInput('');
  ...
  function call() {
    ...
    if (stage === 'dial') {
      if (input !== USSD_CODE) {
        setScreen(`Code inconnu. Composez ${USSD_CODE}`);
        setStage('ended');
        setInput('');
        return;
      }
  ...
  return (
    <div id="panel-ussd" role="tabpanel" aria-labelledby="tab-ussd" className="flex min-h-0 flex-1 flex-col bg-[#1b2622] p-3 text-white">
      <div className="min-h-[190px] flex-1 overflow-y-auto rounded-2xl bg-[#e3f1eb] p-4 font-mono text-[15px] text-[#06291f]" aria-live="polite">
        {loading ? (
          <p className="flex items-center gap-2"><Loader2 size={16} className="animate-spin" aria-hidden /> Connexion USSD…</p>
        ) : stage === 'dial' ? (
          <div className="space-y-3">
            <p className="text-sm">Composez le code puis appuyez sur la touche verte.</p>
        ...
        {stage === 'session' && !loading && <p className="mt-3 border-t border-[#0b4f3c]/30 pt-2 text-xs">Tapez votre choix puis la touche verte.{acc ? ` Saisie : ${acc}` : ''}</p>}
      </div>
      <p className="num my-2 min-h-8 text-center text-2xl tracking-widest" aria-label="Saisie">{input || (stage === 'dial' ? '' : ' ')}</p>
      <div className="grid grid-cols-3 gap-2">
        ...
        <button type="button" onClick={hangUp} className="grid min-h-12 place-items-center rounded-2xl bg-white/25" aria-label="Raccrocher">
        ...
        <button
          ...
          aria-label={stage === 'dial' ? 'Appeler' : 'Envoyer'}
        >
          <PhoneCall size={22} aria-hidden />
        </button>
        <button type="button" onClick={() => setInput((v) => v.slice(0, -1))} className="grid min-h-12 place-items-center rounded-2xl bg-white/10" aria-label="Effacer">
