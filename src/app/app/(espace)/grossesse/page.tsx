...
export default async function GrossessePage() {
  ...
  if (!p) {
    return (
      <>
        <PageHead icon="pregnant" title="Ma grossesse" />
        {res.error ? <ErrorNote error={res.error} /> : <Empty>Aucun suivi de grossesse en cours. À la première consultation prénatale, la sage-femme l’ouvre dans votre carnet.</Empty>}
      </>
    );
  }
  ...
  const listen = [
    `Vous êtes à ${p.weeksAmenorrhea} semaines d'aménorrhée, au ${p.trimester === 1 ? 'premier' : p.trimester === 2 ? 'deuxième' : 'troisième'} trimestre.`,
    `Le bébé est attendu vers le ${fmtDate(p.edd, { day: 'numeric', month: 'long' })}.`,
    next ? `Prochaine consultation : ${next.label}, ${visitWindow(next.dueFrom, next.dueTo, 'long')}.` : '',
  ...
  return (
    <>
      <PageHead icon="pregnant" title="Ma grossesse" listen={listen} audioKey="app.grossesse" />
      ...
      <section aria-labelledby="h-terme" className="grid gap-4 sm:grid-cols-3">
        <div className="card flex flex-col items-center justify-center gap-1 !border-0 bg-[var(--color-brand-900)] p-6 text-center text-white sm:col-span-1">
          <h2 id="h-terme" className="label !text-[var(--color-brand-200)]">Semaines d’aménorrhée</h2>
          <p className="num text-7xl font-bold leading-none">{p.weeksAmenorrhea}</p>
          <p className="text-lg">{p.termLabel}</p>
        </div>
        <div className="card grid grid-cols-2 gap-3 p-5 sm:col-span-2">
          <div className="rounded-2xl bg-[var(--bg)] p-4">
            <p className="label">Trimestre</p>
            <p className="num text-4xl font-bold">{p.trimester}<span className="text-xl">{p.trimester === 1 ? 'er' : 'e'}</span></p>
          </div>
          <div className="rounded-2xl bg-[var(--bg)] p-4">
            <p className="label">Terme prévu</p>
            <p className="text-2xl font-bold">{fmtDate(p.edd, { day: 'numeric', month: 'long' })}</p>
            <p className="text-base text-[var(--fg-muted)]">{p.daysToTerm > 0 ? `dans ${p.daysToTerm} jours` : 'terme atteint'}</p>
          </div>
          <div className="col-span-2 flex items-center gap-3 rounded-2xl bg-[var(--color-brand-100)] p-4 text-[var(--color-brand-900)]">
            ...
            <p className="text-lg">
              {next ? (
                <>
                  <strong>Prochaine visite :</strong> {next.label}, {visitWindow(next.dueFrom, next.dueTo, 'short')}
                  {p.maternity ? ` · ${p.maternity}` : ''}
                </>
              ) : (
                'Toutes les consultations prénatales sont faites. Bravo !'
              )}
            </p>
          </div>
        </div>
      ...
      <Section id="h-danger" title="Signes de danger" icon="warning">
        ...
        {p.recordedDangerSigns.length > 0 && (
          <details className="simple-hide mt-4">
            <summary className="cursor-pointer py-2 font-bold">Signes déjà signalés ({p.recordedDangerSigns.length})</summary>
            <ul className="mt-2 space-y-1 text-base">
              {p.recordedDangerSigns.map((s) => (
                <li key={`${s.code}-${s.at}`}>
                  {fmtDate(s.at, { day: 'numeric', month: 'long' })} · <strong>{s.label}</strong> · signalé par {s.reportedBy}
                </li>
              ))}
            </ul>
          </details>
      ...
      <Section id="h-cpn" title="Mes consultations prénatales" icon="calendar" className="simple-hide">
        {p.visits.some((v) => v.status === 'en retard') && (
          <div className="mb-4">
            <Notice tone="warn" title="Une consultation est en retard">Allez au centre de santé dès que possible : ce n’est pas trop tard.</Notice>
          </div>
        )}
              ...
              <li key={v.id} className="relative flex gap-4">
                ...
                <div className={`min-w-0 flex-1 rounded-2xl p-4 ${st.box}`}>
                  <p className="flex flex-wrap items-center gap-2">
                    <span className="pill bg-white/70 text-[var(--color-ink)] ring-1 ring-black/10">{st.text}</span>
                    <span className="text-base font-bold">
                      {v.doneAt ? `le ${fmtDate(v.doneAt, { day: 'numeric', month: 'long' })}` : `du ${fmtDate(v.dueFrom, { day: 'numeric', month: 'short' })} au ${fmtDate(v.dueTo, { day: 'numeric', month: 'short' })}`}
                    </span>
                  </p>
                  <p className="mt-1 text-lg font-bold">{v.label}</p>
                  {v.place && (
                    <p className="flex items-center gap-1 text-base"><MapPin size={16} aria-hidden /> {v.place}</p>
                  )}
                  {v.content.length > 0 && v.status !== 'faite' && (
                    <details className="mt-2">
                      <summary className="cursor-pointer text-base font-bold">Ce qui sera fait</summary>
                      <ul className="mt-1 list-disc pl-5 text-base">
                        {v.content.map((c) => <li key={c}>{c}</li>)}
                      </ul>
                    </details>
                  )}
                </div>
...
function visitWindow(from: string, to: string, month: 'long' | 'short') {
  const a = fmtDate(from, { day: 'numeric', month });
  const b = fmtDate(to, { day: 'numeric', month });
  return a === b ? `le ${a}` : `entre le ${a} et le ${b}`;
}
