...
export function SymptomForm({ patientId }: { patientId?: string }) {
  ...
  async function send(severity: number) {
    ...
    try {
      ...
      const r = await sendOrQueue<Result>({ path: '/care/symptoms', method: 'POST', body: { symptom, severity, ...(patientId ? { patientId } : {}) }, label: `Symptôme : ${label}` });
    ...
    if (result.queued) {
      return (
        <div role="status" className="space-y-3 rounded-3xl bg-[var(--color-ocre-100)] p-5 text-[var(--color-ocre-700)]">
          <p className="text-xl font-bold">Pas de réseau : c’est noté sur le téléphone.</p>
          <p>L’envoi à votre équipe se fera tout seul au retour du réseau. Si c’est grave, n’attendez pas : allez aux urgences ou appelez le 118.</p>
          <button type="button" className="btn btn-ghost" onClick={reset}>Noter autre chose</button>
        </div>
      );
    }
    return (
      <div role="alert" className="space-y-4">
        <div className={`rounded-3xl p-5 ${result.alert ? 'bg-[var(--color-danger-600)] text-white' : 'bg-[var(--color-brand-100)] text-[var(--color-brand-900)]'}`}>
          <p className="text-2xl font-bold">{result.alert ? 'Signe d’alerte : votre équipe est prévenue' : 'C’est noté dans votre carnet'}</p>
          {result.alert && result.notified > 0 && <p className="mt-1 text-lg">{result.notified} soignant{result.notified > 1 ? 's' : ''} de votre équipe ont reçu un message.</p>}
        </div>
        <ul className="list-disc space-y-1 pl-6 text-lg">
          {result.advice.map((a) => <li key={a}>{a}</li>)}
        </ul>
        <div className="flex flex-wrap gap-3">
          {result.alert && <a href="tel:118" className="btn btn-danger"><Phone size={20} aria-hidden /> Appeler le 118</a>}
          {result.alert && <Link href="/app/carte-urgence" className="btn btn-ghost">Ma carte d’urgence</Link>}
          <button type="button" className="btn btn-soft" onClick={reset}>Noter autre chose</button>
        </div>
      </div>
    );
  }
  ...
  if (!symptom) {
    return (
      <fieldset>
        <legend className="mb-3 text-lg font-bold">1. Qu’est-ce que vous ressentez ?</legend>
  ...
  return (
    <fieldset className="space-y-4" disabled={busy}>
      <legend className="mb-3 flex items-center gap-3 text-lg font-bold">
        ...
        2. {chosen?.label} : c’est fort comment ?
      </legend>
      ...
      <button type="button" className="btn btn-ghost" onClick={() => setSymptom(null)}>
        <ArrowLeft size={20} aria-hidden /> Changer de symptôme
      </button>
    </fieldset>
  );
}
