...
export function ShareFlow() {
  ...
  async function create() {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : 'Pas de réseau : le partage demande une connexion. Réessayez.');
    } finally {
      setBusy(false);
    }
  }
  ...
  if (phase.kind === 'waiting') {
    ...
    return (
      <div className="flex flex-col items-center gap-5 text-center">
        <p className="text-xl font-bold">Montrez ce code au soignant</p>
        <div className="rounded-3xl bg-white p-3 shadow-sm ring-1 ring-[var(--border)]">
          <QrClient value={share.qrPayload} size={280} label="QR de partage de votre carnet, à scanner par le soignant" />
        </div>
        <div>
          <p className="label">Ou dictez ce code</p>
          <p className="num text-5xl font-bold tracking-[.2em]" aria-label={`Code : ${share.shareCode.split('').join(' ')}`}>
        ...
        <p className="flex items-center gap-2 rounded-full bg-[var(--color-ocre-100)] px-4 py-2 font-bold text-[var(--color-ocre-700)]">
          <Clock size={18} aria-hidden /> À scanner avant {hourOnly(share.scanBefore)}
        </p>
        <p role="status" aria-live="polite" className="text-base text-[var(--fg-muted)]">
          En attente du scan… Accès prévu pour {share.hours} h : {scopes.map((s) => SCOPE_LABEL[s].toLowerCase()).join(', ')}
          {sensitive ? ', données très sensibles' : ''}.
        </p>
        <button type="button" className="btn btn-ghost" onClick={() => void cancel(share)}>Annuler ce partage</button>
      </div>
    );
  }
  ...
  if (phase.kind === 'granted') {
    ...
    return (
      <div role="status" className="space-y-4 text-center">
        ...
        <p className="text-2xl font-bold">{consent.grantee ?? 'Le soignant'} a maintenant accès jusqu’à {fmtDateTime(consent.expiresAt)}.</p>
        <p className="text-[var(--fg-muted)]">Vous pouvez retirer cet accès à tout moment, juste en dessous.</p>
        <button type="button" className="btn btn-soft" onClick={() => setPhase({ kind: 'choose' })}>Faire un autre partage</button>
      </div>
    );
  }
    ...
    >
      ...
      <fieldset className="mt-4">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {SCOPES.map((s) => (
            <label key={s.key} className="flex min-h-14 cursor-pointer items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-3 has-[:checked]:border-[var(--color-brand-500)] has-[:checked]:bg-[var(--color-brand-50)] has-[:checked]:text-[var(--color-brand-950)]">
              ...
              <span>
                <span className="block font-bold">{SCOPE_LABEL[s.key]}</span>
        ...
        <label className="mt-3 flex cursor-pointer items-start gap-3 rounded-2xl border border-dashed border-[var(--color-ocre-500)] p-3">
          ...
          <span>
            <span className="flex items-center gap-2 font-bold"><ShieldAlert size={18} aria-hidden /> Données très sensibles</span>
        ...
        <div className="grid grid-cols-3 gap-2">
          {DURATIONS.map((h) => (
            <button key={h} type="button" aria-pressed={hours === h} onClick={() => setHours(h)} className={`btn !min-h-14 text-lg ${hours === h ? 'btn-primary' : 'btn-ghost'}`}>
              {h === 1 ? '1 heure' : h === 24 ? '24 heures' : '3 jours'}
            </button>
          ))}
        </div>
      </fieldset>
      ...
      <button type="submit" className="btn btn-primary w-full !min-h-14 text-lg" disabled={busy || scopes.length === 0}>
        <QrCode size={22} aria-hidden /> {busy ? 'Préparation…' : 'Montrer mon QR au soignant'}
      </button>
      {scopes.length === 0 && <p className="text-base text-[var(--fg-muted)]">Cochez au moins une partie du carnet.</p>}
    </form>
  );
}
