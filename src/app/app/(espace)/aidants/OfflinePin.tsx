...
export function OfflinePin() {
  ...
  async function seal() {
    if (!PIN_RE.test(pin)) return setMsg({ ok: false, text: 'Le code PIN doit avoir 4 à 6 chiffres.' });
    if (pin !== pin2) return setMsg({ ok: false, text: 'Les deux codes ne sont pas pareils.' });
    setBusy(true);
    setMsg(null);
    try {
      ...
      setPin('');
      setPin2('');
      setMsg({ ok: true, text: 'Copie protégée enregistrée. Vous pourrez l’ouvrir sans réseau avec votre code PIN.' });
    } catch (e) {
      setMsg({ ok: false, text: e instanceof ApiError ? e.message : 'Il faut du réseau pour préparer la copie. Réessayez.' });
    } finally {
      setBusy(false);
    }
  }
  ...
  async function unlock() {
    ...
    try {
      ...
      if (n >= MAX_FAILS) {
        ...
        setFails(0);
        setSealed(false);
        setMsg({ ok: false, text: 'Trop d’erreurs : la copie a été effacée pour vous protéger. Recréez-la avec du réseau.' });
      } else {
        setFails(n);
        setMsg({ ok: false, text: `Code PIN incorrect. Encore ${MAX_FAILS - n} essai${MAX_FAILS - n > 1 ? 's' : ''} avant effacement de la copie.` });
      }
    } finally {
      setBusy(false);
    }
  ...
  function wipe() {
    if (!window.confirm('Effacer de ce téléphone la copie protégée du carnet et la carte d’urgence ?')) return;
    wipeLocal();
  ...
  if (opened) {
    ...
    return (
      <div className="space-y-3">
        <p role="status" className="text-base text-[var(--fg-muted)]">Copie hors ligne du {fmtDateTime(opened.at)}</p>
        <dl className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl bg-[var(--color-danger-50)] p-4 text-[var(--color-danger-800)]"><dt className="text-sm font-bold">Groupe sanguin</dt><dd className="num text-4xl font-bold">{s.bloodGroup ?? '?'}</dd></div>
          <div className="rounded-2xl bg-[var(--bg)] p-4"><dt className="label">Allergies</dt><dd className="font-bold">{s.allergies.length ? s.allergies.join(', ') : 'Aucune connue'}</dd></div>
          <div className="rounded-2xl bg-[var(--bg)] p-4"><dt className="label">Traitement</dt><dd className="font-bold">{s.treatments ?? 'Aucun renseigné'}</dd></div>
          <div className="rounded-2xl bg-[var(--bg)] p-4"><dt className="label">Maladies suivies</dt><dd className="font-bold">{s.conditions.map((c) => c.label ?? c.code).join(', ') || 'Aucune'}</dd></div>
          <div className="rounded-2xl bg-[var(--bg)] p-4"><dt className="label">Personne à prévenir</dt><dd className="font-bold">{s.emergencyContact ? `${s.emergencyContact.name} ${s.emergencyContact.phone ?? ''}` : 'Non renseignée'}</dd></div>
          <div className="rounded-2xl bg-[var(--bg)] p-4">
            <dt className="label">Rendez-vous</dt>
            <dd className="font-bold">{s.nextReminders.length ? s.nextReminders.map((r) => `${r.title} (${fmtDate(r.dueAt, { day: 'numeric', month: 'short' })})`).join(' · ') : 'Aucun'}</dd>
          </div>
        </dl>
        <button type="button" className="btn btn-primary" onClick={() => setOpened(null)}><Lock size={20} aria-hidden /> Refermer</button>
      </div>
    );
  }
      ...
      {sealed ? (
        <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); void unlock(); }}>
          <label className="block">
            <span className="mb-1 block font-bold">Code PIN pour ouvrir la copie hors ligne</span>
          ...
          <div className="flex flex-wrap gap-3">
            <button type="submit" className="btn btn-primary" disabled={busy || unlockPin.length < 4}><LockOpen size={20} aria-hidden /> {busy ? 'Ouverture…' : 'Ouvrir'}</button>
            <button type="button" className="btn btn-ghost" onClick={wipe}><Trash2 size={20} aria-hidden /> Effacer de ce téléphone</button>
          </div>
        </form>
      ) : (
      ...
      <details className="rounded-2xl bg-[var(--bg)] p-4" open={!sealed}>
        ...
        <form className="mt-3 space-y-3" onSubmit={(e) => { e.preventDefault(); void seal(); }}>
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="mb-1 block font-bold">Nouveau code (4 à 6 chiffres)</span>
            ...
            <label className="block">
              <span className="mb-1 block font-bold">Le même code encore</span>
