...
export function AddDelegation() {
  ...
  async function submit() {
    ...
    try {
      ...
      setMsg({ ok: true, text: 'Aidant ajouté. Il reçoit un SMS (sans aucune donnée médicale).' });
      setPhone('');
      setRelation('');
      router.refresh();
    } catch (e) {
      setMsg({ ok: false, text: e instanceof ApiError ? e.message : 'Pas de réseau. Réessayez.' });
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    ...
    >
      <h3 className="text-lg font-bold">Ajouter un aidant</h3>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block">
          <span className="mb-1 block font-bold">Son numéro de téléphone</span>
        ...
        <label className="block">
          <span className="mb-1 block font-bold">Qui est-ce pour vous ?</span>
          <input className="input" list="relations" placeholder="mère, fils, voisine…" value={relation} maxLength={30} onChange={(e) => setRelation(e.target.value)} required minLength={2} />
          <datalist id="relations">
            {['mère', 'père', 'époux', 'épouse', 'fils', 'fille', 'frère', 'sœur', 'ami', 'voisin'].map((r) => <option key={r} value={r} />)}
          </datalist>
        </label>
      </div>
      <fieldset>
        <legend className="mb-2 font-bold">Ce qu’il peut faire</legend>
          ...
          {SCOPES.map((s) => (
            <label key={s.key} className="flex min-h-14 cursor-pointer items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-3">
              ...
              <span>
                ...
                <span className="block text-sm text-[var(--fg-muted)]">{s.hint}</span>
              </span>
            </label>
          ))}
        </div>
        <p className="mt-2 text-sm text-[var(--fg-muted)]">Un aidant ne voit jamais vos données très sensibles.</p>
      </fieldset>
      {msg && (
      ...
      <button type="submit" className="btn btn-primary w-full" disabled={busy || scopes.length === 0 || phone.replace(/\D/g, '').length < 8 || relation.trim().length < 2}>
        <UserPlus size={20} aria-hidden /> {busy ? 'Ajout…' : 'Ajouter cet aidant'}
      </button>
    </form>
  );
}
...
export function DiscreetToggle({ initial }: { initial: boolean }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <p role="status" className="text-base font-bold">{on ? 'Activé' : 'Désactivé'}</p>
