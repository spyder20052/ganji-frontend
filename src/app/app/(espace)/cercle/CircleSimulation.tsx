    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        <button type="button" className="btn btn-primary" disabled={running} onClick={() => { setShown(0); setRunning(true); }}>
          <Play size={20} aria-hidden /> Simuler : rappel de 8 h non confirmé
        </button>
        {shown > 0 && !running && (
          <button type="button" className="btn btn-ghost" onClick={() => setShown(0)}><RotateCcw size={20} aria-hidden /> Effacer</button>
        )}
      </div>
      <ol className="space-y-3" aria-live="polite">
        {STEPS.slice(0, shown).map(({ at, Icon, t, d }, i) => (
