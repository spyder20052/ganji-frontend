...
export const metadata: Metadata = { title: 'Cercle de soin' };
...
export default function CerclePage() {
  return (
    <>
      <PageHead
        icon="people"
        title="Cercle de soin"
        intro="Plusieurs proches et le relais du quartier, chacun avec son rôle. Quand un rappel n’est pas confirmé, le cercle prend le relais."
        listen="Le cercle de soin réunit vos aidants, le relais communautaire et votre équipe de soins. Si vous ne confirmez pas un rappel, un aidant est prévenu, sans détail médical."
        audioKey="app.cercle"
      >
        <PreviewBadge />
      </PageHead>
      ...
      <Section id="h-cercle" title="Qui fait partie du cercle" icon="people">
          ...
          {CIRCLE.map((c) => (
            <li key={c.name} className={`flex items-center gap-3 rounded-2xl border p-4 ${c.lead ? 'border-[var(--color-brand-500)] bg-[var(--color-brand-50)] text-[var(--color-brand-950)]' : 'border-[var(--border)]'}`}>
              ...
              <span className="min-w-0">
                ...
                <span className="block text-base">{c.role}</span>
                <span className="block text-sm opacity-80">{c.does}</span>
              </span>
            </li>
          ))}
        </ul>
      ...
      <Section id="h-escalade" title="Rappel non confirmé : aidant prévenu" icon="calendar">
        <CircleSimulation />
      </Section>

      <Section id="h-visites" title="Visites du relais" icon="home">
        <ul className="space-y-2">
          {VISITS.map((v) => (
            <li key={v.date} className="flex flex-wrap items-center gap-3 rounded-2xl bg-[var(--bg)] p-4">
              <span className={`pill ${v.done ? 'bg-[var(--color-brand-100)] text-[var(--color-brand-900)]' : 'bg-[var(--card)] ring-1 ring-[var(--border)]'}`}>{v.done ? 'Faite' : 'Prévue'}</span>
              <span className="font-bold">{v.date}</span>
              <span className="text-[var(--fg-muted)]">· {v.who}</span>
              <p className="w-full text-base">{v.what}</p>
            </li>
          ))}
        </ul>
      </Section>
