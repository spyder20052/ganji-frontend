...
export default async function PatientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (e instanceof ServerApiError && e.status === 403) return <Refusal patientId={id} />;
      ...
      <header className="flex flex-wrap items-end gap-x-4 gap-y-2">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[var(--fg-muted)]">
            {summary.age} ans · {summary.sex === 'F' ? 'femme' : 'homme'} · né{summary.sex === 'F' ? 'e' : ''} le {fmtDate(summary.birthDate)}
            {summary.commune ? ` · ${summary.commune}` : ''}
          </p>
        </div>
      </header>
      <AccessBanner access={summary.access} />
      ...
      <div className="grid items-start gap-5 lg:grid-cols-2">
        <section aria-labelledby="h-timeline" className="card p-5">
          <h2 id="h-timeline" className="text-xl font-bold">
            Chronologie de soins
          </h2>
          {timeline === null ? (
            <NotShared what="la chronologie" />
          ) : timeline.length === 0 ? (
            <p className="mt-3 text-[var(--fg-muted)]">Aucun évènement enregistré.</p>
          ) : (
            <ol className="relative mt-4 space-y-4 border-l-2 border-[var(--border)] pl-5">
              {timeline.map((t) => {
                const k: { icon: LucideIcon; label: string; danger?: boolean } = KIND[t.kind] ?? { icon: FileText, label: t.kind };
                return (
                  <li key={`${t.kind}-${t.id}`} className="relative">
                    ...
                    {t.detail && <p className="mt-0.5 whitespace-pre-line text-[0.95rem]">{t.detail}</p>}
                    {t.author && <p className="text-sm text-[var(--fg-muted)]">{t.author}</p>}
                  </li>
                );
              })}
            </ol>
        ...
        <section aria-labelledby="h-obs" className="card p-5">
          ...
          {series === null ? (
            <NotShared what="les résultats d’analyses" />
          ) : sortedSeries.length === 0 ? (
            <p className="mt-3 text-[var(--fg-muted)]">Aucun résultat enregistré.</p>
          ) : (
            <div className="mt-4 space-y-6">
              {sortedSeries.map((s) => (
                <div key={s.code}>
                  <h3 className="flex items-baseline justify-between gap-2 font-bold">
                    {s.label} <span className="text-sm font-normal text-[var(--fg-muted)]">{s.unit} · {s.points.length} mesure{s.points.length > 1 ? 's' : ''}</span>
                  </h3>
                  <LineChart series={s} height={160} />
                </div>
              ))}
            </div>
          )}
...
function AccessBanner({ access }: { access: Summary['access'] }) {
  if (access.via === 'BREAK_GLASS') {
    return (
      <p role="note" className="flex items-start gap-3 rounded-2xl border border-[var(--color-ocre-500)]/50 bg-[var(--color-ocre-100)] p-3 text-[var(--color-ocre-700)]">
        ...
        <span>
          <strong>Accès d’urgence (bris de glace){access.expiresAt ? ` ${until(access.expiresAt)}` : ''}.</strong> Motif enregistré ; le patient, ses proches et le contrôleur ont été prévenus. Chaque lecture est journalisée.
        </span>
      </p>
    );
  }
  const text =
    access.via === 'CARE_TEAM'
      ? 'Accès : équipe de soins déclarée du patient.'
      : access.via === 'CONSENT'
        ? `Accès : consentement du patient${access.expiresAt ? `, ${until(access.expiresAt)}` : ''}.`
        : `Accès : ${access.via.toLowerCase()}.`;
  return (
    <p role="note" className="flex items-start gap-3 rounded-2xl bg-[var(--color-brand-100)] p-3 text-[var(--color-brand-900)]">
      ...
      <span>
        <strong>{text}</strong> Le patient voit cette consultation dans son journal d’accès.
      </span>
    </p>
  );
}

function VitalCard({ s }: { s: Summary }) {
  return (
    <section aria-labelledby="h-vital" className="card grid gap-5 p-5 md:grid-cols-[auto_1fr_1fr]">
      ...
      <div className="flex flex-row items-center gap-4 md:flex-col md:items-start">
        <div className="grid h-24 w-28 place-items-center rounded-3xl bg-[var(--color-danger-50)] text-[var(--color-danger-800)]">
          <span className="label !text-[var(--color-danger-800)]">Groupe</span>
          <span className="num -mt-2 text-4xl font-bold">{s.bloodGroup ?? '?'}</span>
        </div>
        {s.emergencyContact && (
          <div className="text-sm">
            <p className="label">Contact d’urgence</p>
      ...
      <div className="space-y-4">
        <div>
          ...
          {s.allergies.length ? (
            ...
            <p className="mt-1">Aucune allergie connue</p>
          )}
        </div>
        <div>
          <p className="label">Traitements en cours</p>
          <p className="mt-1 whitespace-pre-line">{s.treatments || 'Aucun traitement déclaré'}</p>
        </div>
      </div>

      <div className="space-y-4">
        <div>
          <p className="label">Maladies et antécédents</p>
          {s.conditions.length ? (
            <ul className="mt-1 space-y-1">
              {s.conditions.map((c) => (
                <li key={c.id}>
                  <span className="font-bold">{c.label ?? c.code}</span>
                  {c.since && <span className="text-sm text-[var(--fg-muted)]"> · depuis {fmtDate(c.since, { month: 'short', year: 'numeric' })}</span>}
                </li>
              ))}
            </ul>
          ) : (
            <p className="mt-1">Aucun antécédent déclaré</p>
          )}
          {s.hiddenSensitive && (
            <p className="mt-2 flex items-center gap-2 text-sm text-[var(--fg-muted)]">
              <EyeOff size={16} aria-hidden /> Des informations très sensibles sont masquées (compartiment séparé, non partagé).
            </p>
          )}
        </div>
        {s.careTeam.length > 0 && (
          <div>
            <p className="label">Équipe de soins</p>
            <ul className="mt-1 space-y-0.5 text-sm">
              {s.careTeam.map((m) => (
                <li key={`${m.name}-${m.role}`}>
                  <span className="font-bold">{m.name}</span> · {SPECIALTY_LABEL[m.specialty] ?? m.specialty}
...
function NotShared({ what }: { what: string }) {
  return (
    <p className="mt-3 flex items-center gap-2 text-[var(--fg-muted)]">
      <Lock size={16} aria-hidden /> Le patient n’a pas partagé {what} avec vous.
    </p>
  );
}
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <section role="alert" className="card flex items-start gap-4 border-2 !border-[var(--fg)] p-6">
        ...
        <div>
          <h1 className="text-2xl font-bold">Accès refusé : le patient n’a pas donné son accord.</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <section aria-labelledby="h-ask" className="card space-y-3 p-5">
          ...
          <h2 id="h-ask" className="text-lg font-bold">
            Demander au patient de montrer son QR
          </h2>
          <p className="text-[var(--fg-muted)]">
        ...
        <section aria-labelledby="h-bg" className="card space-y-3 border-[var(--color-danger-600)]/30 p-5">
          ...
          <h2 id="h-bg" className="text-lg font-bold">
            Urgence vitale et patient hors d’état de consentir
          </h2>
          <p className="text-[var(--fg-muted)]">Le bris de glace ouvre le dossier 12 h. Il est justifié, tracé et contrôlé.</p>
