...
export function OpenRecord() {
  ...
  return (
    <section aria-labelledby="h-open" className="card space-y-4 p-5">
      <div>
        <h2 id="h-open" className="text-xl font-bold">Ouvrir un carnet</h2>
        <p className="text-[var(--fg-muted)]">Le patient montre son QR de partage (ou vous dicte son code). L’accès est limité dans le temps et il le voit dans son journal.</p>
      </div>
      <QrScanner
        onValue={redeem}
        busy={busy}
        inputLabel="Saisir le code à 6 chiffres"
        placeholder="482 913"
        inputHint="Code affiché sous le QR du patient, valable 15 minutes. Vous pouvez aussi coller le contenu du QR."
        submitLabel="Ouvrir"
      />
