  ...
  async function alertDonors() {
    ...
    try {
      ...
      setNote(
        res.alerted
          ? res.alerted > 1
            ? t('{n} donneurs de plus alertés.', { n: res.alerted })
            : t('{n} donneur de plus alerté.', { n: res.alerted })
          : t('Aucun autre donneur compatible disponible à moins de 40 km.'),
      );
      setExpanded(true);
      await load();
    } catch (e) {
      ...
      <div className="flex flex-wrap items-start gap-x-4 gap-y-2">
        ...
        <div className="min-w-0 flex-1 basis-64">
          ...
          <p className="num mt-1 text-sm">
            <span className="font-bold">{r.counts.alerted}</span> {t('alertés')} · <span className="font-bold text-[var(--color-brand-700)]">{t('{n} oui', { n: r.counts.accepted })}</span> · {t('{n} non', { n: r.counts.declined })} · {t('{n} en attente', { n: r.counts.waiting })}
          </p>
        </div>
                      ...
                      <tr key={d.id}>
                        ...
                        <td className="py-1.5 pr-2">
                          <span className="inline-flex items-center gap-1.5 text-sm">
                            <Icon size={16} aria-hidden /> {t(CHANNEL_LABEL[d.channel] ?? d.channel)}
                          </span>
                        </td>
                        <td className="py-1.5">
